...
if isempty(yLim) || ismember(panel,plotSettings.adjustLimsToData)
    switch panel
        ...
        case 'vel'  % velocity
            lim = [0 max(cellfun(@(x)max(x,[],'omitnan'),plotData{2}),[],'omitnan')];
        case 'pup'  % pupil
            lim = [0 max(cellfun(@(x)max(x,[],'omitnan'),plotData{2}),[],'omitnan')];
        otherwise
            % just take range of data
            mins = cellfun(@(x)min(x,[],'omitnan'),plotData{2});
            maxs = cellfun(@(x)max(x,[],'omitnan'),plotData{2});
            lim = [min(mins,[],'omitnan') max(maxs,[],'omitnan')];
    end
